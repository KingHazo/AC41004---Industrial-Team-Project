...
if (empty($accountNumber) || empty($holderName) || $amount <= 0 || !is_numeric($amount)) {
    ...
    echo json_encode(['success' => false, 'message' => 'Invalid deposit details or amount.']);
    exit();
}
...
try {
    ...
    if ($stmt_investor->rowCount() === 0) {
        throw new Exception("Investor record not found or update failed.");
    }
    
    $sql_bank = "UPDATE Bank 
                 SET Balance = Balance - :amount
                 WHERE AccountNumber = :accountNumber AND HolderName = :holderName AND Balance >= :amount"; // Added check to prevent negative balance
    ...
    if ($stmt_bank->rowCount() === 0) {
        throw new Exception("Bank account not found or insufficient funds for withdrawal.");
    }
    ...
    // success response
    echo json_encode(['success' => true, 'message' => 'Deposit successful!', 'new_balance_added' => $amount]);
