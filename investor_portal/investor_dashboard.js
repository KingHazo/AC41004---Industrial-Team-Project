// View pitch (investor view) - FIX: Capture PitchID and redirect correctly
document.querySelectorAll('.view-btn').forEach(btn => {
    btn.addEventListener('click', (event) => {

        const pitchId = event.currentTarget.getAttribute('data-id');
    
        if (pitchId) {

            window.location.href = `investor_pitch_details.php?id=${pitchId}`;
            } else {
                console.error("Pitch ID not found for 'View Pitch' button.");
                alertMessage('Cannot view pitch details. Missing Pitch ID.', 'error');
            }
        });
    });
...
// handle the actual deletion with fetch API
    ...
    document.querySelectorAll('.cancel-btn').forEach(btn => {
        btn.addEventListener('click', (event) => {
            if (event.currentTarget.hasAttribute('disabled')) {
                alertMessage('This investment cannot be cancelled because the pitch is already funded or closed.', 'error');
                return;
            }

            // Get the InvestmentID
            const investmentId = event.currentTarget.getAttribute('data-id');
            
            if (investmentId) {
                // show confirmation that will call deleteInvestment on yes
                showConfirmation(
                    'Are you sure you want to permanently cancel this investment? Funds will be returned to your balance.',
